...
namespace Gdn.Tests
{
    public abstract class GdnTestBase : AbpIntegratedTestBase<GdnTestModule>
    {
        ...
        protected void UsingDbContext(Action<GdnDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<GdnDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<GdnDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<GdnDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<GdnDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GdnDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<GdnDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<GdnDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<GdnDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<GdnDbContext, Task<T>> func)
        {
            T result;
